...
#include "C:\Users\Lex-alviur\Desktop\New folder\max30101_new.h.h"
...
int max30101_sample_fetch(uint8_t *fifo_data, uint8_t size,I2C_HandleTypeDef hi2c1)
{

	uint8_t buffer[MAX30101_MAX_BYTES_PER_SAMPLE];
	uint8_t bufferRead;
	uint8_t bufferWrite;
	uint8_t buffDiff[5]={0};
//	int fifo_chan;
//	int num_bytes;
//	int i;
	...
	returnValue = HAL_I2C_Master_Receive(&hi2c1, MAX30101_I2C_ADDRESS<<1, &bufferRead,	4,100);
	...
	if(cont3<5){
	buffDiff[cont3] = bufferWrite - bufferRead;
	...
	returnValue = HAL_I2C_Master_Transmit(&hi2c1, MAX30101_I2C_ADDRESS<<1, &MAX30101_REG_FIFO_DATA, 1,100);
	
	if (returnValue != HAL_OK) {
		return SENSOR_ERROR;
	}
		
	returnValue = HAL_I2C_Master_Receive(&hi2c1, MAX30101_I2C_ADDRESS<<1, fifo_data,	size, 100);
	
	if (returnValue != HAL_OK) {
		return SENSOR_ERROR;
	}
	
	fifo_data[0] = fifo_data[0] & 0x03;
...
int max30101_init(I2C_HandleTypeDef hi2c1)
{
...
int max30101_off(I2C_HandleTypeDef hi2c1){
	uint8_t dataBuffer[2] = {0, 0};
...
int max30101_on(I2C_HandleTypeDef hi2c1){
	uint8_t dataBuffer[2] = {0, 0};
